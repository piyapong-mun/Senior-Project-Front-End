...
export async function GET(req: Request) {
  try {
    ...
    if (!activityId || !moduleId || !quizId) {
      return NextResponse.json(
        { ok: false, message: "activity_id, module_id, and quiz_id are required" },
        { status: 400 }
      );
    }
    ...
    const res = await fetch(`${BACKEND}/activity/student/course/quiz/${activityId}/${stdId}/${moduleId}/${quizId}`, {
      method: "GET",
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Failed to fetch quiz progress: ${errText}`);
    }
